...
#import "SBFacePigSnoutView.h"
@interface SBFacePigSnoutView()
@property (nonatomic,readwrite) UIImageView *faceHeadView;
@end
@implementation SBFacePigSnoutView

- (UIImageView *)faceHeadView{
    if (_faceHeadView == nil) {
        _faceHeadView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth*0.7, 150)];
        _faceHeadView.image = [UIImage imageNamed:@"猪鼻子"];
...
- (void)drawFaceView:(NSArray *)faces{
    if (faces.count == 1) {
        @autoreleasepool{
            ...
            self.faceHeadView.center = CGPointFromString(((NSString *)strPoints[12]));
